...
export default async function sitemap():Promise<MetadataRoute.Sitemap> { const [contests,news]=await Promise.all([getContests(),getNewsSitemapEntries()]); const routes=["","/quem-somos","/produtos","/presencial","/plano-de-combate","/concursos","/concursos/lista","/noticias","/parceiros","/glossario","/editais","/materiais-gratuitos",...courseCategories.map((category)=>`/produtos/${category.slug}`)].map((path)=>({url:`${siteUrl}${path}`,changeFrequency:"daily" as const,priority:path===""?1:.8})); return [...routes,...contests.map((contest)=>({url:`${siteUrl}/concursos/${contest.slug}`,lastModified:contest.lastVerifiedAt,changeFrequency:"weekly" as const,priority:.7})),...news.map((article)=>({url:`${siteUrl}/noticias/${encodeURIComponent(article.routeKey)}`,lastModified:article.lastModified,changeFrequency:"weekly" as const,priority:.65}))]; }
